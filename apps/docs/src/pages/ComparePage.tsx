    ...
    <div>
      <div style={{ ...docCardStyle(), padding: 24, marginBottom: 24 }}>
        ...
        <p style={docBodyStyle()}>
          ...
          <code>isAnimationActive={'{false}'}</code>. Chart data is a{" "}
          <strong>synthetic load generator</strong> — not real ops telemetry. Performance numbers
          are measured on your device using isolated <code>flushSync</code> calibration (same
          methodology as <code>apps/bench-harness</code>), with alternating live ticks for ongoing
          per-library samples.
        </p>
        <p style={{ ...docBodyStyle(), marginBottom: 0 }}>
          Published CI reference (Chromium 4×, <strong>6 panels × 2000 pts</strong>): AxiCharts{" "}
          <strong>2.9 ms</strong> · Recharts <strong>54.3 ms</strong> · ECharts 26.5 ms — shown in
          the demo when the Standard preset matches that fixture. Try <strong>Heavy</strong> or{" "}
          <strong>Extreme</strong> presets to widen the gap on your hardware. See{" "}
